...
def choose_featured_snippet(cleaned_value, cleaned_value_for_query, rows, special_result, full_query_with_full_stops, session, nlp):
	...
	keyword_values = {"what is", "code", "markup", "meetup", "event", "review", "recipe", "what are", "what were", "why", "how", "microformats", "who is"}
	...
	if search_for_snippet == True:
		...
		cleaned_value_for_query = cleaned_value.replace("what is", "").replace("code", "").replace("markup", "").replace("what are", "") \
			.replace("why", "").replace("how", "").replace("what were", "").replace("to use", "").strip()
	...
	if "who is" in cleaned_value or ("." in cleaned_value and len(cleaned_value.split(".")[1]) <= 4) \
		or cleaned_value.endswith("social") or cleaned_value.endswith("get rel") or cleaned_value.endswith("inspect feed"):
