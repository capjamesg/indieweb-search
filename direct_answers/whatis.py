def parse_what_is(original_cleaned_value, soup, url, direct):
    ...
    if (
        ...
        p_tags = [
            ...
            if ("p-summary" in p.attrs.get("class", []) and p.text != None)
            or "{} is".format(original_cleaned_value) in p.text.lower()
            or "{} are".format(original_cleaned_value) in p.text.lower()
        ]
