...
def generate_featured_snippet(cleaned_value, special_result, nlp, url=None, post=None, direct=False):
    ...
    do_i_use, response = whois.parse_feed(original_cleaned_value, soup, url, original_url)

    if do_i_use != None and response != None:
        return do_i_use, response

    do_i_use, response = whois.parse_address(original_cleaned_value, soup, url, original_url)
    ...
    if len(original_cleaned_value) != 0 and len(original_cleaned_value.split(" ")) > 0 and "what is" in original_cleaned_value.lower():
        try:
            ...
            special_result = {"type": "direct_answer", "breadcrumb": url, "title": soup.find("h1").text, "featured_image": post.get("featured_image")}
        except:
            do_i_use = ""
    else:
        do_i_use = ""
