    ...
    if "recipe" in original_cleaned_value:
        ...
        if h_recipe:
            ...
            if len(ingredients) > 0 and len(instructions) > 0:
                return "<h3>Ingredients</h3><p>{}</p><h3>Instructions</h3><p>{}</p>".format("".join(i for i in ingredients), "".join(i for i in instructions)), {"type": "direct_answer", "breadcrumb": url, "title": post["title"]}
