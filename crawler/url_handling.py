...
def crawl_urls(
    ...
    if "javascript:" in full_url:
        print(
            "{} marked as noindex because it is a javascript resource".format(full_url)
        )
        return url, {}, False, feeds, None, crawl_depth, average_crawl_speed

    if "/wp-json/" in full_url:
        print(
            "{} marked as noindex because it is a wordpress api resource".format(
                full_url
            )
        )
        return url, {}, False, feeds, None, crawl_depth, average_crawl_speed
    ...
    if len(in_matching_namespace) < 2:
            ...
            for link in header_links:
                if link["rel"] == "canonical":
                    if link["rel"].startswith("/"):
                        ...
                        link["rel"] = (
                            full_url.split("/")[0] + "//" + domain + link["rel"]
                        )
        ...
        if page.status_code == 404 or page.status_code == 410:
            ...
            or page.status_code == 302
            or page.status_code == 308
        ):
            ...
            if redirected_to:
                ...
                if discovered_urls.get(full_url) and discovered_urls.get(
                    full_url
            ...
            url_handling_helpers.check_remove_url(full_url)
            print("{} page {} status code".format(full_url, page.status_code))
            return url, {}, False, feeds, None, crawl_depth, average_crawl_speed
        ...
        print(
            "{} took {} seconds to load".format(full_url, page.elapsed.total_seconds())
        )
        ...
        if page_desc_soup.find("meta", {"name": "rating"}):
            if (
                page_desc_soup.find("meta", {"name": "rating"}).get("content")
                == "adult"
                or page_desc_soup.find("meta", {"name": "rating"}).get("content")
                == "RTA-5042-1996-1400-1577-RTA"
            ):
                print(
                    "{} marked as adult content in a meta tag, skipping".format(
                        full_url
                    )
                )
                return (
        ...
        if nofollow_all == False:
            links = [
                ...
                or (
                    not l.get("rel")
                    and l.get("href") not in ["#", "javascript:void(0);"]
                )
            ]
        else:
            links = []
            ...
            if link_headers:
                ...
                for link_header in link_headers:
                    if 'rel="hub"' in link_header and len(feeds) < 25:
                        websub_hub = (
                            link_header.split(";")[0].strip().strip("<").strip(">")
                        )
                        ...
                        feeds.append(
                            {
                                ...
                                "mime_type": "websub",
                                "etag": "NOETAG",
                                "discovered": datetime.datetime.now().strftime(
                                    "%Y-%m-%d"
                                ),
                            }
                        )
                        ...
                        print(
                            "{} is a websub hub, will save to feeds.json".format(
                                websub_hub
                            )
                        )
        ...
        if (
            "/Special:" in full_url
            and "MediaWiki"
            in page_desc_soup.find("meta", {"name": "generator"})["content"]
        ):
        ...
        if (
            ...
            print(
                "{} marked as follow, noindex because it is a page archive".format(
                    full_url
                )
            )
            url_handling_helpers.check_remove_url(full_url)
        ...
        if page_text.text and len(page_text.text.split(" ")) < 100:
            print(
                "content on {} is thin (under 100 words), marking as thin_content".format(
                    full_url
                )
            )
            thin_content = True
